...
router.post("/saveNotes", fetchuser, async (req, res) => {
  ...
  if (saveNotes) {
    success = true;
    return res.status(200).json({ success, message: "notes save sucessfully" });
  } else {
    res.status(400).json({ message: "something Wrong!" });
  }
});

router.get("/getmynotes", fetchuser, async (req, res) => {
  const getNotes = await Pdf.find({ user: req.userData.id });
  if (!getNotes) {
